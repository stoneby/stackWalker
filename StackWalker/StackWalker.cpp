/*StackWalker.cpp : 定义 DLL 应用程序的导出函数。*/
//
#include "stdafx.h"
#include <windows.h>
#include <tchar.h>
...
void StackWalker::ReleaseSTACKFRAME64(void* p)
{
	STACKFRAME64* stackFrame = (STACKFRAME64*)p;
	delete stackFrame;
}
...
BOOL StackWalker::LoadModules()
{
  ...
  if (m_modulesLoaded != FALSE && m_ReloadPDB == FALSE)
    return TRUE;
...
STACKFRAME64* GetSTACKFRAME64(CONTEXT& c)
{
	// init STACKFRAME for first call
	STACKFRAME64* sAgent = new STACKFRAME64(); // in/out stackframe
	STACKFRAME64& s = *sAgent;
	memset(&s, 0, sizeof(s));
...
BOOL StackWalker::ShowCallStackOnlyFrame( HANDLE hThread /*= GetCurrentThread()*/, const CONTEXT *context /*= NULL*/, PReadProcessMemoryRoutine readMemoryFunction /*= NULL*/, LPVOID pUserData /*= NULL /* optional to identify some data in the 'readMemoryFunction'-callback */ )
{
	...
	STACKFRAME64* sTemp = GetSTACKFRAME64(c);
	STACKFRAME64& s = *sTemp;
	for (frameNum = 0; ; ++frameNum )
	{
		...
		if(frameNum > 2)
		{
			STACKFRAME64* sAgent = new STACKFRAME64();
			*sAgent = s;
			if (s.AddrReturn.Offset == 0)
			{
				//this->OnCallStackEntryOnlyFrame(sAgent);
...
void StackWalker::FlushStackFrams(void* p)
{
	std::list<STACKFRAME64*>* frames = (std::list<STACKFRAME64*>*)p;
	std::list<STACKFRAME64*>::iterator it = frames->begin();
	std::list<STACKFRAME64*>::iterator end = frames->end();
	for (; it != end; ++it)
	{
		STACKFRAME64* tempP = *it;
		FlushStack((void*)tempP);
	}
}

void StackWalker::FlushStack(void* frame)
{
	STACKFRAME64* tmpFrame = (STACKFRAME64*)frame;
	STACKFRAME64& s = *tmpFrame;
	...
	if (s.AddrPC.Offset != 0)
	{
		...
		else
		{
			...
			{
				this->OnDbgHelpErr("SymGetSymFromAddr64", lastError, s.AddrPC.Offset);
			}
		}
...
void StackWalker::OnCallStackEntryOnlyFrame( void* frame )
{
...
void StackWalker::OnCallstackEntry(CallstackEntryType eType, CallstackEntry &entry)
{
  ...
  {
	if(strcmp(entry.moduleName,"ntdll") == 0)
		  return;
	if(strcmp(entry.moduleName,"kernel32") == 0)
		return;
    if (entry.name[0] == 0)
